...
const EventDashboard = () => {
  ...
  const [showJoinLimitPopup, setShowJoinLimitPopup] = useState(false); // Define the state for the join limit popup
  ...
  return (
    <>
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>{editEvent ? "Edit Event" : "Create Event"}</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form
            onSubmit={editEvent ? handleUpdateEvent : handleSubmit}
            className="mt-3"
          >
